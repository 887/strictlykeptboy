...
#
# Output: release/strictlykeptboy-<version>-<sha7>.apk and a release/latest.apk symlink.
#
# Signing: this uses Gradle's debug keystore by default (good for personal
# sideload). For a real published release, set STRICTLYKEPTBOY_RELEASE_KEYSTORE +
# STRICTLYKEPTBOY_RELEASE_KEY_ALIAS + STRICTLYKEPTBOY_RELEASE_KEY_PASSWORD env vars and the
...
OUT_APK="${OUT_DIR}/strictlykeptboy-${TAG}.apk"
mkdir -p "${OUT_DIR}"

# Decide release vs debug build flavour
if [ -n "${STRICTLYKEPTBOY_RELEASE_KEYSTORE:-}" ]; then
    echo "[build-release-apk] release-signed build (keystore: ${STRICTLYKEPTBOY_RELEASE_KEYSTORE})"
    GRADLE_TASK="assembleRelease"
    BUILD_APK="app/build/outputs/apk/release/app-release.apk"
else
    echo "[build-release-apk] debug-signed build (set STRICTLYKEPTBOY_RELEASE_KEYSTORE for production signing)"
...
\`\`\`
adb install -r strictlykeptboy-${TAG}.apk
\`\`\`
...
\`\`\`
sha256sum strictlykeptboy-${TAG}.apk
# expected: ${APK_SHA256}
\`\`\`
EOF
    ...
    if gh release view "${REL_TAG}" >/dev/null 2>&1; then
        ...
        gh release create "${REL_TAG}" "${OUT_APK}" \
            --title "strictlykeptboy ${VERSION} (${SHA7})" \
