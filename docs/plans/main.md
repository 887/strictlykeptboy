...
- [x] **DDD.12** `ui/scaffold/ModePill.kt` — always-visible mode-pill composable with long-press → transition modal + typed-confirmation gate per UI-TT. Not yet hooked into `SkbAppShell` top-bar (UI wiring is the AVD-loop follow-up).
- [x] **DDD.13** `ui/reviews/ReviewsPane.kt` — Dom-side / Boy-side surface with filter chips, reaction picker (9 tokens), free-text composer, cute-coded LGTM render. Not yet added as a top-bar destination (avoids regressing the existing 2 `AppShellNavigationSwapTest` failures).
