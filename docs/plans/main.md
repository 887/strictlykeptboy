...
- Unticked `[ ]` total: **162**
  - **Round 3 — must-have** (production v1 close-out): **54**
  - **Round 4 — should-have** (substantial feature work): **63**
  - **v2 — nice-to-have** (polish + niche): **45**

  _Re-classified 2026-05-13 from a single aspirational "Round 3 backlog"
  bucket into three honest buckets. The original wishful "everything is
  Round 3" tagging is gone — Round 3 now scopes only what genuinely
  closes out production-shippable v1 or finishes ≥80%-done Round-2
  phases._
...
- [ ] **A.5** Licensee plugin — deferred to Phase LL (Round 2 license audit); not needed at scaffold time (deferred — gated on Phase LL; Round 3 — must-have)
...
- [ ] **C.8** Validation: refuse to write malformed entries; surface validation errors to UI — DEFERRED to Phase DM-H follow-up (Phase C scope-trim). Writer currently relies on caller-side construction guarantees. (deferred — DM-H follow-up; Round 3 — must-have)
...
- [ ] **N.3** Tap slot → quick-create event (target repo + calendar picker) — stub v1; full editor sheet deferred to I-K-EE (deferred — wire Together-slot → `EventCreateSheet` from Phase FFF; Round 3 — must-have)
...
- [ ] **P.3** CSV export for tasks — deferred to Phase S (tasks shipped tests don't yet exercise an exporter; revisit alongside Phase S.6). (deferred — small follow-up; v2 — nice-to-have)
- [ ] **P.4** iCal export for tasks (VTODO) — bonus, deferred. (deferred — bonus; v2 — nice-to-have)
- [ ] **P.5** `tools/` directory with shell scripts for server-side CalDAV/Thunderbird/Outlook hooks — deferred; not in v1 scope. (deferred — out of v1 scope; v2 — nice-to-have)
...
- [ ] **Q.4** Voice prompts ("what's next?") (v2 — nice-to-have — overlaps with Phase HH Auto voice-create)
...
- [ ] **X.8** Bundle in release pipeline alongside APK (GitHub Releases asset; homebrew tap `887/tap/skb`; `curl ... | sh` one-liner) (deferred — release-pipeline polish; Round 3 — must-have)
- [ ] **X.9** AGENTS.md content references CLI as primary path (rewrite per DM-K of data-model.md extensions) (deferred — docs rewrite; Round 3 — must-have)
...
- [ ] **Y.8** `skb caldav add|sync|remove|list` subcommands — covers all three modes; `--mode pull|push|bidi` flag at add-time (Round 3 — must-have)
- [ ] **Y.9** Per-CalDAV-mirror sync interval (default 30m for mirrors) (Round 3 — must-have)
- [ ] **Y.10** ETag- + CTag- + RFC6578 `sync-token`-based incremental change detection (avoid full re-pull) (Round 3 — must-have)
- [ ] **Y.11** Mirror-source attribution in event detail UI: small "↻ mirrored from <server-host>" line + last-sync timestamp + link to re-sync now (Round 3 — must-have)
- [ ] **Y.12** AGENTS.md note for repos with mirrored calendars: "the `work` calendar is a CalDAV mirror; edits to its files will be overwritten on next pull — change events upstream instead" (deferred — docs note; Round 3 — must-have)

## Phase Z — Git LFS

- [ ] **Z.1** JGit LFS config wiring (`.gitattributes` for `attachments/**` over threshold) (Round 4 — should-have)
- [ ] **Z.2** Threshold-based auto-routing (default 1MB, configurable per repo) (Round 4 — should-have)
- [ ] **Z.3** Provider capability detection (probe LFS endpoint; on failure, fall back to in-tree) (Round 4 — should-have)
- [ ] **Z.4** Fallback warning UI on no-LFS providers ("this provider does not support LFS; large attachments stored inline") (Round 4 — should-have)
- [ ] **Z.5** Migration helper: convert existing repo's large in-tree attachments to LFS (`skb migrate --lfs`) (Round 4 — should-have)
...
- [ ] **AA.1** Per-event `tz_id` field (additive, optional, non-breaking) (Round 4 — should-have)
- [ ] **AA.2** Repo default tz in `repo.toml` (Round 4 — should-have)
- [ ] **AA.3** Display-tz toggle in top bar; per-event "pin to event tz" flag (Round 4 — should-have)
- [ ] **AA.4** Multi-tz common-time finder (Round 4 — should-have)
- [ ] **AA.5** DST edge-case test corpus (Round 4 — should-have)
- [ ] **AA.6** `skb tz convert <event-id> <new-tz>` CLI subcommand (Round 4 — should-have)
- [ ] **AA.7** Participant-tz declaration in Together-tab common-time UI (Round 4 — should-have)

## Phase BB — Weather overlay

- [ ] **BB.1** Open-Meteo client integration (Apache-2.0 lib) (Round 4 — should-have)
- [ ] **BB.2** Per-repo location setting + location-permission flow (Round 4 — should-have)
- [ ] **BB.3** Cache layer (per-(location, date) day forecast, refresh every 6h) (Round 4 — should-have)
- [ ] **BB.4** Day view weather strip (3h granularity) (Round 4 — should-have)
- [ ] **BB.5** Week view header icons (Round 4 — should-have)
- [ ] **BB.6** Month view day-cell icons (Round 4 — should-have)
- [ ] **BB.7** Per-event location override (for travel events) (Round 4 — should-have)
- [ ] **BB.8** CC-BY attribution in About screen (Round 4 — should-have)
- [ ] **BB.9** `skb weather show <date>` CLI subcommand (Round 4 — should-have)
...
- [ ] **CC.1** Schema: `events/<yyyy>/<mm>/<event-id>.comments/<comment-id>.md` (Round 4 — should-have)
- [ ] **CC.2** Event detail comments UI section (list + add) (Round 4 — should-have)
- [ ] **CC.3** `comments` notification channel + per-event mute toggle (D.37) (Round 4 — should-have)
- [ ] **CC.4** `skb comment add|list` CLI subcommands (Round 4 — should-have)
- [ ] **CC.5** Comment author chip rendering (same shape as event chip) (Round 4 — should-have)
- [ ] **CC.6** Threaded `in_reply_to` rendering (indented under parent) (Round 4 — should-have)

## Phase DD — Drag-to-reschedule + pinch-to-zoom

- [ ] **DD.1** Long-press detector on event chips (Day + Week views) (Round 4 — should-have)
- [ ] **DD.2** Drag gesture with grid snapping (configurable grid; default 15min) (Round 4 — should-have)
- [ ] **DD.3** Drop commit with auto-message `move event "<title>" from <old> to <new>` (Round 4 — should-have)
- [ ] **DD.4** Pinch-to-zoom timeline (5min / 15min / 30min / 1h levels) (Round 4 — should-have)
- [ ] **DD.5** Zoom-level persistence per device (Round 4 — should-have)
- [ ] **DD.6** Recurrence drag: prompt "this instance only / this and future / entire series" — creates appropriate exception or rule edit (Round 4 — should-have)
...
- [ ] **FF.1** Pack format spec (directory + optional `pack.toml`) (v2 — nice-to-have)
- [ ] **FF.2** Install from local picker (zip or unzipped dir) (v2 — nice-to-have)
- [ ] **FF.3** Install from URL fetch (v2 — nice-to-have)
- [ ] **FF.4** `:sticker-name:` shortcut in title editors with autocomplete (v2 — nice-to-have)
- [ ] **FF.5** Icon picker integration (repos, calendars, todolists) (v2 — nice-to-have)
- [ ] **FF.6** Settings → Appearance → Sticker packs (list, install, remove) (v2 — nice-to-have)

## Phase GG — Signed commits (optional capability)

- [ ] **GG.1** GPG private-key import flow (file picker + paste-armored-text) (v2 — nice-to-have)
- [ ] **GG.2** Per-identity signing-key picker (lists imported key fingerprints) (v2 — nice-to-have)
- [ ] **GG.3** JGit + BouncyCastle signing wiring (PGP signature on commits when toggle on) (v2 — nice-to-have)
- [ ] **GG.4** Verified-author chip on entries from signed commits (small verified-checkmark badge) (v2 — nice-to-have)
- [ ] **GG.5** "Off by default" — confirmed throughout UI and CLI (v2 — nice-to-have)
- [ ] **GG.6** Key revocation flow (remove imported key; stops signing; existing signed commits unaffected) (v2 — nice-to-have)
- [ ] **GG.7** Per-repo signing-key picker (override per repo if the active identity has multiple keys) (v2 — nice-to-have)

## Phase HH — Android Auto voice-create

- [ ] **HH.1** Voice intent registration (`com.eight87.strictlykeptboy.action.CREATE_EVENT`) (v2 — nice-to-have)
- [ ] **HH.2** Natural-language parsing (date / time / title / calendar) — use Android's `Recognizer` + simple regex; fall back to ChatGPT-style fuzzy if a future ML model is wired in (v2 — nice-to-have)
- [ ] **HH.3** Default-repo + default-calendar resolution (v2 — nice-to-have)
- [ ] **HH.4** Confirm-by-voice flow ("create event 'dentist' tomorrow at 3pm in Personal — say yes to confirm") (v2 — nice-to-have)

## Phase II — Cross-device snooze sync (opt-in)

- [ ] **II.1** Setting toggle: Settings → Notifications → "Sync snoozes across devices" (v2 — nice-to-have)
- [ ] **II.2** `_local/snoozes.toml` schema (`[[snooze]]` entries with `event_id`, `until`, `device_id`) (v2 — nice-to-have)
- [ ] **II.3** Auto-merge resolver for snoozes (latest-wins per entry; idempotent) (v2 — nice-to-have)
- [ ] **II.4** Conflict-free idempotency verification (write a snooze, write the same snooze, confirm no double-entry) (v2 — nice-to-have)

## Phase JJ — Multi-branch awareness

- [ ] **JJ.1** Repo switcher shows current branch under repo name (v2 — nice-to-have)
- [ ] **JJ.2** Branch picker in repo settings (v2 — nice-to-have)
- [ ] **JJ.3** Branch creation flow (`skb branch create <name>`; UI form) (v2 — nice-to-have)
- [ ] **JJ.4** Branch switching (with stash + reset path for uncommitted local changes) (v2 — nice-to-have)
- [ ] **JJ.5** PR deep-link to provider's compare page (v2 — nice-to-have)
- [ ] **JJ.6** `skb branch list|create|switch` CLI subcommands (v2 — nice-to-have)

## Phase KK — CSV import for tasks

- [ ] **KK.1** Settings → Templates → "Import tasks from CSV" flow (Round 4 — should-have)
- [ ] **KK.2** Column mapping UI (auto-detect common names, user confirms unknowns) (Round 4 — should-have)
- [ ] **KK.3** `skb task import-csv <file> --list <todolist>` CLI subcommand (Round 4 — should-have)
- [ ] **KK.4** Idempotency on re-import (by row hash; subsequent imports update existing entries) (Round 4 — should-have)

## Phase LL — License audit + release scope

- [ ] **LL.1** Audit every dep against the no-GPL constraint: (deferred — license-audit completion; Round 3 — must-have)
...
- [ ] **LL.2** Licensee plugin allowlist updated for Round 2 deps (deferred — license-audit completion; Round 3 — must-have)
- [ ] **LL.3** About screen license list extended (deferred — license-audit completion; Round 3 — must-have)
...
- [ ] **MM.A1** Register intent filter for `strictlykeptboy://add?...` (custom scheme) (Round 3 — must-have)
- [ ] **MM.A2** Register intent filter for `https://strictlykeptboy.app/add?...` (universal link) (Round 3 — must-have)
- [ ] **MM.A3** Digital Asset Links JSON at `https://strictlykeptboy.app/.well-known/assetlinks.json` for verified App Links (Round 3 — must-have)
- [ ] **MM.A4** URL parser: extract `url`, `label`, `mode`, `priority`, `via`, `references` query params (Round 3 — must-have)
- [ ] **MM.A5** URL fragment parser: extract `token`, `expires` (never sent over network) (Round 3 — must-have)
- [ ] **MM.A6** Multi-URL support (`?url=A&url=B`) (Round 3 — must-have)
- [ ] **MM.A7** Token-wipe: clear `#token=` from any persisted referrer after consumption (Round 3 — must-have)
- [ ] **MM.A8** Expiry enforcement: refuse links past `expires` (Round 3 — must-have)
- [ ] **MM.A9** QR scan integration (ZXing already in for D.42; verify reuse) (Round 3 — must-have)
...
- [ ] **NN.5** "Offer to add referenced repos" sheet UI (per-reference toggle) — deferred to UI-GG. Schema fields shipped to consume. (deferred — UI-GG follow-up; Round 3 — must-have)
...
- [ ] **OO.1** State-file schema (done, snooze, note, reaction, priority-override, mute, hide) (Round 4 — should-have)
- [ ] **OO.2** Source-repo-id derivation (SHA-256 of normalized URL → 16 hex) (Round 4 — should-have)
- [ ] **OO.3** Resolver merge: source entity ⊕ state file → rendered instance (Round 4 — should-have)
- [ ] **OO.4** Writer atomicity: state-file commits one file per state change (Round 4 — should-have)
- [ ] **OO.5** `_local/state/` for pre-own-repo state (migrated on repo creation per D.47) (Round 4 — should-have)
- [ ] **OO.6** `skb state set --done|...` CLI surface (Round 4 — should-have)
- [ ] **OO.7** State-file orphan detection: when source entity is deleted, prompt to clean up state files (Round 4 — should-have)

## Phase PP — Simplified mode chrome

Deep-dives: [`shared-schedules.md`](shared-schedules.md) SH-E, [`ui-spec.md`](ui-spec.md) UI-FF.

- [ ] **PP.1** Mode state in app prefs (`simplified` | `full`) (Round 4 — should-have)
- [ ] **PP.2** Hidden surfaces: repo management, identities, templates, advanced sync, multi-view tabs, Together tab. When the active repo is no-origin (per Phase ZZ.G), ALL remote/sync UI is hidden including the sync button in the top bar; the sync-status badge becomes a permanent "local" badge. (Round 4 — should-have)
- [ ] **PP.3** Visible surfaces: Schedule (single view), Tasks (combined), sync button, comment composer, settings (minimal) (Round 4 — should-have)
- [ ] **PP.4** "Switch to full mode" entry point in settings (one-tap-reversible) (Round 4 — should-have)
- [ ] **PP.5** Mode-label picker (Simplified / Focused / Received Schedules / Good Boy / Good Girl / Good Pet / Kept / Other) (Round 4 — should-have)
- [ ] **PP.6** Auto-entry: simplified by default when only read-only repos OR only no-origin repos are configured AND the user has no own remote repo (per Phase ZZ.G). (Round 4 — should-have)
- [ ] **PP.7** Play Store screenshots use "Simplified" label exclusively (Round 4 — should-have)
...
- [ ] **QQ.1** Detect "launched via deep-link AND no repos configured" condition. First-launch routing (per Phase K LW-A.4): if launched via `strictlykeptboy://add?...` deep-link and no repos configured → this QQ bootstrap. Otherwise if no repos configured → Phase K wizard. When launched WITHOUT a deep-link AND no repos AND user picks local-only on the 2-option splash, route to local-only scaffolder via `GitRepo.initLocalOnly` per Phase ZZ.G. (Round 4 — should-have)
- [ ] **QQ.2** Skip wizard; go straight to "Add gifted repo" screen with URL prefilled (Round 4 — should-have)
- [ ] **QQ.3** Auth flow: try `#token=` fragment first, then OAuth, then PAT prompt (Round 4 — should-have)
- [ ] **QQ.4** Clone progress UI with animated mascot (Round 4 — should-have)
- [ ] **QQ.5** Post-clone: boot into simplified mode, schedule view, one-time onboarding card (Round 4 — should-have)
- [ ] **QQ.6** If receiving repo has `references.toml` with `default_active`, prompt to add referenced repos (Round 4 — should-have)
- [ ] **QQ.7** Error handling: bad URL, network down, auth failure — clear messages with retry (Round 4 — should-have)
...
- [ ] **RR.3** Auth method: "Recipient adds own SSH key" path (no token in link) — sender-side SSH-key auth path remains for a future Phase RR.b slice; the existing `mode=read-only` + `mode=read-write` covers the common case and the recipient-accept flow honours both. (deferred per Phase RR slice — Round 3 — must-have)
- [ ] **RR.4** Auth method: "Embed one-shot deploy key" — deferred (provider-API integration). (deferred per Phase RR slice — Round 3 — must-have)
- [ ] **RR.5** Auth method: "Embed fine-grained PAT" — deferred (provider-API integration); `singleUseToken` field carries the agent-readable single-use semantic in the meantime. (deferred per Phase RR slice — Round 3 — must-have)
- [ ] **RR.6** Auth method: "Public repo" (no token needed) — implicit (no `#token=` fragment); UI label deferred. (deferred per Phase RR slice — Round 3 — must-have)
...
- [ ] **RR.8** `skb share` CLI subcommand — deferred; the Phase SS.4 `skb fork` lands here in the same slice but `skb share` itself is out-of-scope for this batch. (deferred per Phase RR slice — Round 3 — must-have)
...
- [ ] **SS.3** Optional template-picker (skippable) — deferred; the fork mechanic preserves the source's template-derived structure verbatim, so the picker is only relevant to the "create from blank" path which is the Phase K wizard. (deferred per Phase SS slice — Round 3 — must-have)
...
- [ ] **SS.6** Mode-choice prompt: stay simplified or switch to full — deferred to the DDD mode-transition slice. (deferred per Phase SS slice — Round 3 — must-have)
- [ ] **SS.7** Reverse path: full mode → "Switch to simplified mode" toggle — deferred to DDD.5. (deferred per Phase SS slice — Round 3 — must-have)
...
- [ ] **TT.1** Local-override (per-device) loaded from state-files at render time (Round 4 — should-have)
- [ ] **TT.2** Repo-modifier from `references.toml` (high/normal/low → ±200) (Round 4 — should-have)
- [ ] **TT.3** Receiver "pin this repo to top" → +500 modifier (Round 4 — should-have)
- [ ] **TT.4** Clamp [1, 1000] with out-of-range warning in broken-entries tray (Round 4 — should-have)
- [ ] **TT.5** Visual indicator on calendars whose displayed priority differs from source-declared priority (Round 4 — should-have)
...
- [ ] **UU.1** Data model: `CalDavOverlay` — no `repoId`, no `targetCalendarId`; events live in an in-memory + Room-cached overlay store keyed by `overlayId`. Additive `CalendarMeta.overlaySource: CalDavOverlayRef?` field on the resolver side. (v2 — nice-to-have)
- [ ] **UU.2** Strict read-only: no push, no edit, no conflict resolution. Long-press / drag / inline-edit refuse with "this calendar lives on a remote server; edit it where you created it." (banner links to the provider's web UI when known) (v2 — nice-to-have)
- [ ] **UU.3** Discovery flow reuses SE-Q PROPFIND chain (well-known → principal → calendar-home → calendar list with checkboxes) (v2 — nice-to-have)
- [ ] **UU.4** Auth reuses SE-Q `CalDavCredential` sealed interface: BasicAuth + OAuth Device Flow (Google/Microsoft) + app-specific-password (Apple) (v2 — nice-to-have)
- [ ] **UU.5** Cache: ETag/CTag/`sync-token` incremental fetch into Room only — **NEVER written to a git repo**, never appears in any `calendars/<id>/events/...` path (v2 — nice-to-have)
- [ ] **UU.6** Resolver integration: overlay calendars are first-class priority calendars alongside repo-backed ones — same priority slider (1..1000), same toggle, same active-windows, same priority-tiebreak comparator (v2 — nice-to-have)
- [ ] **UU.7** Settings UI: `Settings → Calendars → + Add external calendar (CalDAV)` — **top-level** entry, NOT nested under any repo. Distinct from `Settings → Repos → <repo> → + Add CalDAV mirror` (which is the SE-Q mirror surface) (v2 — nice-to-have)
- [ ] **UU.8** CLI: `skb caldav add --overlay <url>` (flag, not a separate `caldav-overlay` subcommand group — keeps surface flat). `skb caldav list` shows both mirrors and overlays with a `kind` column (v2 — nice-to-have)
- [ ] **UU.9** Visual treatment: overlay calendars get a small `🔗 external` badge in the calendar drawer and event list to distinguish from repo-backed (v2 — nice-to-have)
- [ ] **UU.10** Offline behavior: last cached snapshot renders; banner "external calendar last synced Nh ago" when stale beyond 2× sync interval (v2 — nice-to-have)
- [ ] **UU.11** Per-provider rate-limit profiles: Google (1M req/day), MS Graph (throttling-header aware), Apple iCloud (conservative quiet cap), Nextcloud (server-configurable), generic (RFC-compliant defaults). Default overlay poll: **60m** (vs 30m for mirrors — overlays are less interactive) (v2 — nice-to-have)
- [ ] **UU.12** Removal path: drop Room rows + `SecretsStore` entry; no repo cleanup needed because nothing was ever written to disk (v2 — nice-to-have)
- [ ] **UU.13** Test matrix: recorded HTTP fixtures for Google / Microsoft 365 / Apple iCloud / Nextcloud / generic-RFC4791. Live integration tests gated behind a `--live-caldav` flag and skipped in CI until creds are wired (v2 — nice-to-have)
- [ ] **UU.14** Documentation: README + AGENTS.md note that overlay calendars are remote-only and CLI agents reading the repo will NOT see overlay events on the filesystem (v2 — nice-to-have)
...
- [ ] **VV.2** Optional event-frontmatter flag `pin_to_widget = true` (additive; resolver-ignored). Picker mode (a) defaults to the most-recently-pinned event, with all events still browsable. (deferred — VV follow-up; Round 3 — must-have)
...
- [ ] **VV.11** Test fixtures: time-frozen render previews at +30d / +7d / +1d / 0d / -1d (deferred — needs Robolectric harness; Round 3 — must-have).
...
- [ ] **WW.3** Sub-beat support (AV-D). Resolver already accepts `subbeatStickerId` + `subbeatIndex`; CLI + frontmatter array + cross-fade animation pending. (deferred — WW follow-up; Round 3 — must-have)
...
- [ ] **WW.6** Rendering pipeline + cache (AV-H). LRU cache shipped; Coil 2.x + animated WebP + < 300ms first-frame budget remains. (deferred — WW follow-up; Round 3 — must-have)
- [ ] **WW.7** Snapshot test suite (AV-I). Pure-JVM resolver unit tests landed (`DefaultStickerResolverTest`, `PackManifestTest`); Roborazzi harness remains. (deferred — WW follow-up; Round 3 — must-have)
...
- [ ] **BBB.4** UI surfaces per HV-E.3 (schedule hides; manage-overlays renders strikethrough+grey with per-event override toggle; week/month show leaf-glyph). See [`ui-spec.md`](ui-spec.md) UI-PP. *(Deferred — follow-up sub-agent.)* (deferred — BBB UI/notif follow-up; Round 3 — must-have)
...
- [ ] **BBB.6** Attachment renderers per HV-M.4 (per-kind Composable specs: link, qr, file, barcode, vcard, location). See [`ui-spec.md`](ui-spec.md) UI-QQ. *(Deferred — follow-up sub-agent.)* (deferred — BBB UI/notif follow-up; Round 3 — must-have)
...
- [ ] **BBB.10** Multi-reminder firing + notification stacking per HV-N.7 (60s collision window collapses into one Android notification with N expansion lines, privacy-aware). See [`notifications-sharing-import.md`](notifications-sharing-import.md) NS-Z. *(Helper `ReminderCollapsing` shipped; WorkManager firing path deferred — follow-up sub-agent.)* (deferred — BBB UI/notif follow-up; Round 3 — must-have)
- [ ] **BBB.11** System `cal-briefings` calendar per HV-N.6 / D.81: morning + evening briefing events with auto-generated bodies; user disable in Settings → Notifications → "Show briefings". See NS-Z. *(Deferred.)* (deferred — BBB UI/notif follow-up; Round 3 — must-have)
- [ ] **BBB.12** Inverted-habit `post_event_checkin` opt-in per HV-N.8 / D.82. *(Reminder kind shipped; opt-in UI deferred.)* (deferred — BBB UI/notif follow-up; Round 3 — must-have)
- [ ] **BBB.13** Import path extensions per HV-M.6: `.ics` URL/DESCRIPTION → `link` attachments; `.eml` airline confirmation → boarding-pass `barcode` + `file`; `.pkpass` → primary `barcode` + metadata + original archive `file`. See NS-Z import-lift. *(Deferred.)* (deferred — BBB UI/notif follow-up; Round 3 — must-have)
...
- [ ] **CCC.6** Screen 5 — Supersedence picker per HV-F.6 (per-calendar pause toggle defaults; nonSuperseable calendars visually locked; per-event "keep this on" override list materializes `overrides/` files). Sticker: `supersedence-snooze-toggle`. (deferred — CCC full-spec follow-up; Round 3 — must-have)
- [ ] **CCC.7** Screen 6 — Confirm per HV-F.7 (mini-month preview with greyed-strikethrough superseded events; re-edit hop-back). Sticker: `confirm-tail-flick` + `good-boy-stays-good-boy-on-vacation` (neutral `staying-on-track`). _Compact cut renders a text summary card + reassurance line._ (deferred — CCC full-spec follow-up; Round 3 — must-have)
...
- [ ] **CCC.9** Edit-in-flight + cancel-trip per HV-F.9 / HV-F.10 (rehydrate `TripDraft` from existing `cal-trip-<id>/`; diff-commit; cancel deletes the calendar dir + overrides atomically). (deferred — CCC full-spec follow-up; Round 3 — must-have)
...
- [ ] **DDD.5** Migration paths per HV-Q.4 (six locked flows): free→kept-by-AI, free→kept-by-human, kept-by-AI↔kept-by-human, kept→free with 24h cooling-off, kept-by-human↔self-keep, self-keep→kept-by-human. (Two flows wired via `ModePrefs.setMode` + transition-modal typed confirmation; the remaining four — human-dom share + self-keep ramp + kept-by-AI↔kept-by-human — depend on Phase RR share-this-repo flow which is not in this slice.) (deferred — DDD follow-up; Round 3 — must-have)
...
- [ ] **DDD.8** LW Screen 3.5 — Praise + pronouns wizard insert (depends on Phase K.5a wizard work; data side ready via `IdentityTomlData`). (deferred — DDD follow-up; Round 3 — must-have)
...
- [ ] **DDD.11** Agent integration — `IdentityTomlCodec.readOrDefault(repoRoot)` is the read API; alternation logic + briefing salutation + notification bodies remain to wire into NS-* + briefing renderers. (deferred — DDD follow-up; Round 3 — must-have)
...
- [ ] **DDD.14** Dom-persona picker UI — `ModeCategory` already lists the 6 builtins; sub-screen with per-persona register-preview + cadence control + custom-prompt editor + explicit-content gate remains. (deferred — DDD follow-up; Round 3 — must-have)
...
- [ ] **EEE.12** Render-snapshot tests at: cold start / event-active / sub-beat-mid-event / private-event / lockscreen variant (deferred — needs Robolectric harness; pure-JVM unit tests cover the sources + privacy decision matrix in `WidgetCommonTest`; Round 3 — must-have).
- [ ] **EEE.13** AVD smoke (deferred — the autonomous-mode env lacks a running AVD; manifest + APK build cleanly under `:app:assembleDebug`; Round 3 — must-have).
...
### Round 4 — should-have (substantial feature work)
...
### v2 — nice-to-have (polish + niche)

- **Phase FF — Custom sticker / icon packs:** the whole resolver chain works without it; default pack ships in-tree.
- **Phase GG — Signed commits (optional):** GPG key import, signing-key picker, BouncyCastle PGP signing, verified-author chip — off by default; collaborators rarely need this in v1.
- **Phase HH — Android Auto voice-create:** voice intent + NL parsing + confirm-by-voice. Auto already works without voice-create.
- **Phase II — Cross-device snooze sync:** rare + complex; v1 snooze is per-device.
- **Phase JJ — Multi-branch awareness:** advanced git-user surface (branch switching / PR deep-link / `skb branch` CLI).
- **Phase UU — CalDAV overlay (read-only, no-repo):** deferred-by-design per `main.md`; SE-Q mirror already covers the integration case.
- **P.3, P.4, P.5** CSV / VTODO task export + `tools/` shell scripts for server-side hooks (niche export paths).
- **Q.4** Android Auto voice prompts (overlaps with Phase HH, also v2).
