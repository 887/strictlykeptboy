...
8. **God-files past 800 LOC** (R.X.4): `MainActivity.kt` (1884) and `WizardNavHost.kt` (1282) are genuine god-files with multiple unrelated concerns and clean natural seams. `SkbAppShell.kt` (729) extracts cleanly via `SkbAppDestinationContent` split. `AppGraph.kt` (1017) wants 2-3 sub-graphs out (`SystemCalendarGraph`, `AvatarGraph`, `TaskPlaybackGraph`). `AppGraph.kt` portion (#8d): Fixed in commit `b6cdf08` — `AvatarGraph` (avatar pack loaders + resolvers + bitmap cache), `SystemCalendarGraph` (Round 2.18 CalendarContract bridges + system-calendar prefs + AccountManager facade + `buildExternalEventsFlow`), and `TaskPlaybackGraph` (Round 2.16.B tasksViewState + tasksFlow + activeTaskController + projector + transport adapter) all extracted into `composition/`. AppGraph 1041 → 852 LOC. Consumers in MainActivity adapt to `graph.<subgraph>.<field>`. AVD-smoked on `emulator-5558` (Schedule renders, top-bar avatar resolves, NowPlaying mini-sheet works). Screenshot `/tmp/skb-appgraph-subgraphs.png`. `ScheduleDayView.kt` (684), `Entities.kt` (681), `RepoSettingsScreen.kt` (734), and `GitRepo.kt` (674) are cohesive-by-nature — leave alone. `WizardNavHost.kt` portion: Fixed in commit `351634c` — 1282 → 501 LOC; per-screen composables extracted to `ui/wizard/screens/` (Welcome, Species, Identity, Lifestyle, Roles, Templates, Git, Scaffold, Done, ShareWithDom, IdentityPreviewPane); host retains `ProgressRow` + `BatMascotSticker` + `WrappingChipRow` + step dispatcher. `SkbAppShell.kt` portion (#8c): Fixed in commit `26849ef` — `SkbAppDestinationContent` extracted to its own file as an `internal` composable; SkbAppShell.kt drops 729 → 372 LOC and now owns only the composition root + chrome.
...
10. **`ScheduleDayView` takes whole `RenderedSchedule?`** when it needs only `schedule?.days.firstOrNull { it.date == date }`. Sibling views (`ScheduleWeek/Month/Year/Agenda/ThreeDay/Timebox`) have the same shape. Narrow via a `DayBandSource` (`fun get(date: LocalDate): RenderedDay?`) — matches the canonical anti-pattern called out in R.X.1. Fixed in commit `0b257d2` — `fun interface DayBandSource { fun bandsFor(date: LocalDate): List<DayBand> }` in `resolver/DayBandSource.kt` plus an `asDayBandSource()` extension on `RenderedSchedule` (with a `DayBandSource.Empty` sentinel). All seven leaf schedule views (Day / ThreeDay / Week / Month / Year / Agenda / Timebox) now consume `dayBands: DayBandSource` instead of `schedule: RenderedSchedule?`. `SchedulePane` projects the source once via `remember(rendered) { rendered?.asDayBandSource() ?: DayBandSource.Empty }` and passes that to each child (plus a `renderedDates: List<LocalDate>` for the agenda's day-list traversal, since agenda is the only view that needs to *discover* which dates have content). 1253 tests green (the four pre-existing `ZoomLevelRowTest` failures are unrelated). AVD-smoked on `emulator-5558` across Schedule, Day, 3-day, Week, Month, Agenda — every view paints correctly (month grid renders 6×7 with today highlight and accent-tinted chips per cell via 42 per-cell `bandsFor` calls; agenda renders day-grouped event cards; day & 3-day & week timelines render their hour grids). Screenshot `/tmp/skb-schedule-narrow.png`.

**[L] — quick wins (action opportunistically)**

11. `AppGraph` mutable hand-off slots exposed as raw `MutableStateFlow` / `@Volatile var` (`safPermissionRevoked`, `parentPickerHandle`, `defaultWriteRepoName`, `wizardEntryRequest`, `visibleDateRange`) — hide via `StateFlow` + narrow setters. _Fixed in commit `1b095bb`_ — four of five narrowed (private mutable backing + `StateFlow` public + `setX(...)` setter); `wizardEntryRequest` left as `MutableStateFlow` on the public surface because `SkbAppDestinationContent` still clears it to null after the wizard finishes (`wizardEntryRequest?.value = null`); the `SkbAppShell` split (#9) shipped but the clear path landed in `SkbAppDestinationContent` rather than moving into a `ShellCallbacks.onWizardFinished` lambda. Tightening to `StateFlow` is gated on routing that clear through a callback. Sub-graph extraction (#8d) confirmed the slot stays AppGraph-owned (commit `b6cdf08`).
...
15. `resolver/Types.kt` at 465 LOC holds ~12 unrelated data classes — split into `Refs.kt`, `Inputs.kt`, `Outputs.kt`, `ViewMode.kt` when convenient. _Fixed in commit `d9e7a1d`._ — split into `Refs.kt` (id wrappers), `Ranges.kt` (DateRange / HourRange / InstantInterval / ZonedInterval), `Calendars.kt` (CalendarKind / CalendarMeta / ExternalSource / TodolistMeta / RepoSnapshot), `Inputs.kt` (EventInput / RecurrenceInput / ExceptionInput + DeviationKind/Input + OverrideKind/Input — sealed kinds kept next to their Input), `Outputs.kt` (InstanceSource / CompletionState / MaterializedInstance / DayBand / LayeredView / RenderedSchedule / RenderedDay / TimeSlot / TimeWindow), and `ViewMode.kt`. `Types.kt` deleted (no residual). `DayBandSource.kt` from #10 stays in its own file. Same package, all imports unchanged.
