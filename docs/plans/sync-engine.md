...
## Phase SE-Y — No-origin + multi-origin (Round 4)

**See [`draft-no-origin-multi-origin.md`](draft-no-origin-multi-origin.md) (`main.md` Phase ZZ) and `decisions.md` D.74 for the authoritative spec.** This section records the cross-cutting edits that the no-origin / multi-origin layer imposes on existing SE-B / SE-C / SE-D / SE-E / SE-F / SE-I / SE-J / SE-K / SE-L sections.

- [ ] **SE-Y.1** SE-B: replace `"origin"` literals in `fetch` / `pullRebase` / `push` with `RemoteName` parameters; add `addRemote` / `removeRemote` / `renameRemote` / `listRemotes` operations. Add `GitRepo.initLocalOnly(rootDir, authorIdentity)` constructor for no-origin repos. New result variants `FetchResult.NoRemotes`, `PullResult.NoRemotes`, `PushResult.NoRemotes`. Mutex still serializes per-repo via SE-B.10.
- [ ] **SE-Y.2** SE-C / SE-D / SE-E: rekey credentials from `<kind>.<repoId>` to `<kind>.<repoId>.<remoteName>`. One-shot migration rewrites existing keys with `remoteName = "origin"`. Thread-local `(repoId, remoteName)` in `StrictlyKeptBoySshSessionFactory`. `transportConfigCallbackFor(repoId, remoteName)` replaces the per-transport-method factories.
- [ ] **SE-Y.3** SE-F: `RepoConfig` schema migration — `remoteUrl: String` becomes `remotes: List<RemoteBinding>` (may be empty) + `primaryRemote: RemoteName?` (null iff `remotes.isEmpty()`). `RemoteBinding` carries `name`, `url`, `transport`, `authMethod`, `fetchEnabled`, `pushEnabled`, `readOnlyDetected`, `pushPolicy: PUSH|PUSH_LAZY|NEVER`, `displayName`, `fetchRefspec`. `repoId` now UUIDv7 persisted at `.strictlykeptboy/repo-id` (committed to repo per D.74 for cross-device state-file consistency).
- [ ] **SE-Y.4** SE-I: rewrite algorithm. Step 1 = fetch all fetch-enabled remotes (per-remote network errors don't abort others, accumulated). Step 2 = rebase local onto `<primaryRemote>/<branch>`. Step 2.5 = compute divergence vs non-primary remotes; emit `MirrorDivergence(remoteName, count)` signals when commitsAhead > 0. Step 3 = push (SE-Y.5).
- [ ] **SE-Y.5** SE-K: per-remote push policy. Push pass iterates `[primary, then non-primary by add-time]`. `pushPolicy.shouldPushNow(triggerKind)` controls scheduling. Partial-failure semantics: primary success + mirror failure → local commit shipped, mirror enters per-remote retry queue (WorkManager exponential backoff per SE-L), `PartialPushDegraded(remoteName, reason)` signal. Primary failure → block subsequent push attempts to that primary; do NOT push non-primaries. Per-remote `readOnlyDetected`.
- [ ] **SE-Y.6** SE-J: conflict UI labels — "Remote" becomes the primary remote's display label. Diamond-merge mini-flow (adopt-mirror-as-authoritative) reuses the same UI with the mirror's label. **No 4-way conflict UI.**
- [ ] **SE-Y.7** SE-L: error taxonomy — `PushRejected(reason, remoteName)`, `Auth(provider, remoteName)`. Recovery action UI gains per-remote display label. Banner text e.g. "GitHub session for `mirror-family` expired — sign in again".
- [ ] **SE-Y.8** SE-O testing strategy: add no-origin tests (init local, commit, verify no network ops attempted on sync tick) + multi-origin tests (two `git init --bare` directories standing in as two remotes; push/fetch to both; partial-failure scenarios).
- [ ] **SE-Y.9** SE-P invariants: (i) `repo.remotes.isEmpty() ⇒ no foreground SyncService work for that repo`, (ii) `primaryRemote != null ⇔ remotes.isNotEmpty()`, (iii) `repoId is stable across remote-set changes`.
