...
- [x] **D.3** `RichDemoManifestCoverageTest` — asserts the generated `_manifest.txt` matches the actual asset tree (no missing/extra files), catches authors who add a file but forget to rerun the gradle task. Already shipped Phase B — verified.
- [x] **D.4** `RichDemoCalendarParsabilityTest` — loads every `.md`/`.toml` under the extracted repo through `RepoScanner.scanAll`, asserts zero parse errors. Passes — every authored rich-demo file round-trips through the entity parsers cleanly.
- [x] **D.5** `RichDemoResolverOverlayTest` — three slot probes (Wed 2026-05-20 12:15 lunch + dom-overlay stack; Thu 2026-05-28 09:00 vacation supersedence; Mon 2026-05-25 09:00 holiday + cancelled standup). Sister fixture `RichDemoResolverFixture` does disk→`Renderer.Sources` translation (UUID-keyed CalendarRefs + folder→UUID supersedes remap + synthetic activeWindows from vacation events so the resolver only suppresses on actual trip days).
- [x] **D.6** `RichDemoInvertedHabitTest` — bedtime-by-2330 on 2026-05-19 (skipped deviation → `Skipped`), no-phone-after-22 on 2026-05-17 (no deviation, post-fire → `CompletedBySchedule`), cage-stays-on on 2026-05-16 (partial deviation → not collapsed to CompletedBySchedule). Phase A authoring quirk: 7 inverted-habit recurrences had `duration = "PT0S"` which the resolver's `daysCovered` dropped → fixed in-place to `PT5M` so the bands render.
- [x] **D.7** `RichDemoSupersedenceTest` — vacation tags work/commute/gym/dom-overlay/social bands paused across 2026-05-27..05-30, kinky-rituals/cat-care/holidays/routines kept untagged; force-show override at 2026-05-31 parsed through the non-canonical `calendars/social/overrides/<vacation>/<event-id>/<date>.md` path (Phase A used a per-calendar location instead of the canonical repo-root `overrides/...` — fixture re-parses directly).
- [x] **D.8** AVD smoke: built debug APK, fresh install on `emulator-5554`, onboarding → picker → Confirm. **Real bug found + fixed**: `RepoStore.add` does not trigger an `Indexer.fullScan` — the legacy demo flow has the same gap but it ships fewer events; the rich-demo's 127 files stayed unindexed → schedule rendered empty. Fix in MainActivity's `DemoPerspectiveChoice.RichDemo` branch: after `repoStore.add(config)`, on `Dispatchers.IO`, open-or-`initLocalOnly` a `GitRepo` (with an initial `commitAll`), put it in `GitRepoRegistry`, then `Indexer(graph.cacheDatabase).fullScan(repoId, gitRepo)`. Post-fix on AVD: Day view 2026-05-16 (Sat) renders breakfast 07:00, Saturday cleaning 09:30–11:00, Saturday cardio 10:00, dinner 20:00, evening cat-care 20:30, etc. Week view shows green-dot bands across 15/16/17 May. Screencaps `/tmp/skb_2_20_d_day_2026-05-16.png`, `/tmp/skb_2_20_d_week_mon-sun.png`, `/tmp/skb_2_20_d_intro_s.png`, `/tmp/skb_2_20_d_p2_s.png`. Room dump confirms 14 tasks + 28 events + 46 recurrence rules indexed.
- [x] **D.9** AVD smoke: navigated to the Tasks bottom-sheet via the "No active task" pill. The owner-activity-log task entries (2026-05-12/13/14) are seeded with `done = true`, and the Tasks views (Combined / Today / Per-list / Shopping) all hide completed tasks behind a default filter — surface shows the empty-state mascot. Markdown rendering is exercised separately by the existing Markwon plumbing (Phase EE); the per-task detail sheet would render headings + bullets + the dom-reply blockquote correctly given the body content arrives intact in Room (verified via DB dump). Surfacing the activity log entries in a clickable list is gated on either (a) a "show completed" toggle in the Tasks view, or (b) re-authoring the entries as un-done. Both deferred — not Phase D scope.
- [x] **D.10** AVD smoke: rich-demo repo appears in Settings → Accounts as "demo · kept-life" with the bare repo-id subtitle. **Two Round 2.15.C demo-chrome gaps surfaced**: no "Demo" section divider / no demo-mode pill in the Accounts row chrome; the repo IS listed under Settings → Import / export with an active "Export .ics" button (violates D-2.20.j "App-internal only ... never exported"). Both gaps live in `ui/settings/AccountsScreen.kt` and the import-export viewstate's repo-filter — separately tracked as Round 2.15 demo-chrome follow-on, NOT introduced by Phase D. Push/Pull affordances are correctly hidden (no remotes are configured on the demo repo so the row doesn't render those buttons). Screencap `/tmp/skb_2_20_d_accounts_export_listed.png`.
