...
- [ ] **B.1** Add `RichDemoSeeder.kt` at `app/src/main/java/com/eight87/strictlykeptboy/demo/RichDemoSeeder.kt` — `suspend fun seed(parentDir: File, assets: AssetManager)` recursively copies `assets/rich-demo-repo/**` to `parentDir`, preserving the tree.
- [ ] **B.2** Asset enumeration: AssetManager `list()` is non-recursive; ship a generated `assets/rich-demo-repo/_manifest.txt` (newline-separated relative paths, generated by a small gradle task) that the seeder walks. Sub-step: add the gradle task in `app/build.gradle.kts` that regenerates the manifest at build time so authors never have to update it by hand.
- [ ] **B.3** Symlink reconstruction: on extraction, if `.claudemd-is-symlink` marker is present, call `Files.createSymbolicLink(repoRoot/"CLAUDE.md", Paths.get("AGENTS.md"))` and delete the marker.
- [ ] **B.4** Idempotency: add `SharedPreferences` key `pref_rich_demo_seeded_v1` (versioned so future content bumps re-seed); seed only when absent. Wire a `forceReseed` parameter for tests.
- [ ] **B.5** Integration with `DemoRepoSeeder` / `RepoStore`: registering the rich-demo repo with `RepoStore` flagged `isDemo = true` + `kind = "demo"` so the Round 2.15 demo-mode chrome already in place treats it correctly.
- [ ] **B.6** Crash-safe: wrap extraction in a try/finally that deletes partial output on failure so a half-extracted demo never wedges first-launch.
