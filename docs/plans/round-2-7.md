...
## Status: OBSOLETE — superseded by round-2-17-storage-folder.md (mirror model); Phase 2.7.A sticker bake-in shipped in c237174

Phase 2.7.A (sticker pack bake-into-repo) shipped intact. Phases 2.7.B (mirror prefs + SAF picker), 2.7.C (sync-runtime mirror push), 2.7.D (retroactive apply), and 2.7.E (mirror AVD smoke) were superseded by Round 2.17 — the mirror model was replaced with the "parent folder IS the working tree" model (D-2.17.a/b). The partial UI bits that did ship (`MirrorLocation` prefs, SAF picker plumbing, `MirrorReconciler`, `BackupLocationCategory`) were refactored or removed by Round 2.17 Phase A/B/E. D-2.7.b and D-2.7.c carry inline `SUPERSEDED by D-2.17.a/b` annotations below.
...
- **D-2.7.b — Storage choice is a single app-wide setting**, not per-repo. **SUPERSEDED by D-2.17.a — see round-2-17-storage-folder.md.** `MirrorLocation` replaced by `ParentLocation` (Internal / External, never None); repos live as direct subfolders of the chosen parent rather than under `filesDir/repos/` with a mirror. New `RepoStoragePrefs`:
...
- **D-2.7.c — Wizard gets a "Backup location" screen** between `Git` and `Scaffold`. **SUPERSEDED by D-2.17.b — see round-2-17-storage-folder.md.** The wizard now has a "Where should we keep your repos?" Storage step (D-2.17.b/D-2.17.c) using SAF tree URI + real-path resolution; the parent IS the working tree, not a mirror; "skip — decide later" is gone (a parent must be chosen before scaffold). Three options:
...
## Phase 2.7.A — Sticker pack bake-into-repo — shipped in commit c237174
...
## Phase 2.7.B — Mirror prefs + SAF picker + wizard screen — SUPERSEDED by Round 2.17 (partial UI shipped in 28cab13 / de2b984, refactored away by 2.17.A/B)
...
## Phase 2.7.C — Sync-runtime mirror push — SUPERSEDED by Round 2.17 (parent IS the working tree; mirror remote model dropped — see D-2.17.l)
...
## Phase 2.7.D — Retroactive apply — SUPERSEDED by Round 2.17 (`MirrorReconciler` → `ParentReconciler`; stale `mirror` remotes pruned on first reconcile per D-2.17.l)
...
## Phase 2.7.E — AVD smoke + release — SUPERSEDED by Round 2.17 Phase H (AVD smoke H.4 + H.12 covered the migration + parent flow)
