...
/**
 * Phase Q — [CarAppRuntime] is the rendezvous point between the
 * composition root and [SkbCarAppService]. The contract is "writable
 * from one place, readable from the service". This test pins the
 * "fall back to empty source" behaviour the session relies on.
 */
class CarAppRuntimeTest {

    @After fun reset() { CarAppRuntime.todayEventSource = null }
